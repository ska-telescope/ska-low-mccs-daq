...
pushd build || exit
  cmake -DCMAKE_INSTALL_PREFIX=$AAVS_INSTALL -DWITH_CORRELATOR=$COMPILE_CORRELATOR ../cdaq || exit
...
sudo cp $PWD/cdaq/build/acquire_station_beam $AAVS_BIN/
chmod u+x $FILE
...
ln -s $PWD/pydaq/daq_receiver.py $FILE
chmod u+x $FILE
