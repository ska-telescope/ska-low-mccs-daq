    ...
    def __init__(self, tpm_config, log_file):
        self._tests = {'adc': "Check JESD link setting test patterns in the ADCs and verifying data received by FPGAs",
                       ...
                       'f2f': "Check fast data link between FPGAs using FPGA embedded test."
                              "\nWARNING: this test will cause errors in the tile beamformer and 40G interface."
    ...
    def execute(self, iterations=1):
        ...
        for _ in range(iterations):
            ...
            if len(test_result) > 1:
                if all(value == 0 for value in test_result):
                    logging.info("ALL TESTS PASSED!")
                else:
                    if any(value > 0 for value in test_result):
                        logging.info(f"{sum(1 for x in test_result if x > 0)} TESTS FAILED!")
                    if any(value < 0 for value in test_result):
                        logging.info(f"{sum(1 for x in test_result if x < 0)} TESTS SKIPPED!")
            self.log_filter()
            if ret > 0:
                return ret
        return 0
