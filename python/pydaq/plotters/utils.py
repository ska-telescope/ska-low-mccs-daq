...
def get_plotting_parameters(file_manager, conf):
    ...
    if conf.antennas == 'all':
        ...
        if max(antennas_to_plot) >= metadata['n_antennas'] or min(antennas_to_plot) < 0:
            logging.error("Cannot plot antennas {}, file has {} antennas".format(conf.channels, metadata['n_chans']))
            exit(-1)
        else:
    ...
    if conf.polarisations == 'all':
        ...
        if max(pols_to_plot) >= metadata['n_pols'] or min(pols_to_plot) < 0:
            logging.error("Cannot plot pols {}, file has {} pols".format(conf.channels, metadata['n_pols']))
            exit(-1)
        else:
    ...
    if conf.sample_offset >= metadata['written_samples']:
        logging.error("Specified sample offset {} is larger than number of samples in file {}. "
        ...
        plotting_parameters['samples'] = metadata['written_samples'] - conf.sample_offset
        logging.info("Data file contains {} samples".format(plotting_parameters['samples']))

    elif conf.nof_samples + conf.sample_offset > metadata['written_samples']:
        plotting_parameters['offset'] = conf.sample_offset
        plotting_parameters['samples'] = metadata['written_samples'] - conf.sample_offset
        logging.warning("Cannot plot required sample range ({} samples at offset {}). Setting number of samples to "
                        "{}".format(conf.nof_samples, conf.sample_offset, plotting_parameters['samples']))
...
def set_figure_for_timestamps(timestamps, enable_time=False, y_axis=False, rollover=0):
    ...
    if np.any(np.diff(timestamps) < 0):
        ...
        timestamps = np.arange(timestamps[0], 
                               timestamps[0] + len(timestamps) * difference, 
                               difference)
    ...
    # Generate timestamps
    new_timestamps = [md.date2num(datetime.datetime.utcfromtimestamp(x)) for x in timestamps]
...
def get_axes(ax, antenna):
    ...
    if type(ax) == np.ndarray:
        if len(ax.shape) == 1:
            ...
            return ax[old_div(antenna, ax.shape[0]), antenna % ax.shape[1]], \
