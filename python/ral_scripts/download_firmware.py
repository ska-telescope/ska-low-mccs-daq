...
class FirmwareManager:
    ...
    def is_minor_version(self, version):
        return self.split_version_num(version)[2] == 0

    def is_major_version(self, version):
        return self.split_version_num(version)[2] == 0 and self.split_version_num(version)[1] == 0
    ...
    def download_firmware(self, include_ltx=False):
        ...
        for firmware_version in firmware_to_download_list:
            firmware_version_path = Path(str(self.version_to_file(firmware_version)).replace(".bit", ""))
            self.download_firmware_single(firmware_version, firmware_version_path, include_ltx)
    ...
    def download_firmware_single(self, firmware_version, file_path, include_ltx=False):
        shutil.rmtree(self.temp_path, ignore_errors=True)
        self.temp_path.mkdir()
        ...
        with open(os.path.join(self.temp_path, "files.tar.gz"), "wb") as f:
            f.write(response.content)

        with tarfile.open(os.path.join(self.temp_path, "files.tar.gz"), "r:gz") as tar:
            tar.extractall(self.temp_path)

        shutil.copy(self.temp_path/"tpm_firmware.bit", f"{file_path}.bit")

        if include_ltx:
            shutil.copy(self.temp_path/"tpm_firmware.bit", f"{file_path}.ltx")
