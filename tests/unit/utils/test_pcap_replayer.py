...
def interface_fixture() -> str:
    ...
    # whatever the physical network interface is. We get this by looking at the
    # default internet route (0.0.0.0) This automatically filters out docker,
    # loopback, and idle host-only nets. But then we check that the interface
    # is included in the list of interfaces.
    iface = conf.route.route("0.0.0.0")[0]
    if iface not in get_if_list():
        pytest.fail("Default network interface not in list of interfaces")
    return iface
...
def mock_mac_address_fixture(
    ...
    """
    # Use a fixed MAC address for consistent testing. This was generated by
    # https://dnschecker.org/mac-address-generator.php using prefix "daqtst".
    mac_address = "DA:20:FD:3F:EF:BC"
...
def test_pcap_replayer(pcap_replayer: PCAPReplayer, mock_mac_address: str) -> None:
    ...
    :param mock_mac_address: The mock destination mac address.
    ...
    class PacketHandler:
        ...
        def __init__(self) -> None:
            ...
            # mock destination mac address to ensure we only get the packets we
            # are interested in.
            self.num_packets = 0
            self.ready = Event()
            self.sniffer = AsyncSniffer(
                ...
                lfilter=lambda packet: (
                    ...
                    and packet[Ether].dst.lower() == mock_mac_address.lower()
                ),
            )
            self.sniffer.start()
