...
class StationBeamFormatFileManager(AAVSFileManager):
    ...
    def __init__(
        ...
        """
        super(StationBeamFormatFileManager, self).__init__(
            root_path=root_path,
    ...
    def configure(self, file_obj):
        ...
        for polarization in AAVSFileManager.range_array(0, n_pols):
            polarization_grp = file_obj.create_group(
                "polarization_" + str(polarization)
            )
            polarization_grp.create_dataset(
                "data",
                (0, n_chans),
                chunks=(self.resize_factor, 1),
                dtype=self.data_type,
                maxshape=(None, n_chans),
            )
    ...
    def read_data(
        ...
        for part in result:
            ...
            partition_data, partition_timestamps, partition_packets = self._read_data(
                timestamp=timestamp,
            ...
            if concat_cnt < 1:
                ...
                packets_buffer = numpy.concatenate(
                    (packets_buffer, partition_packets), 0
                )
    ...
    def _read_data(
        ...
        **kwargs
        ...
        output_buffer = numpy.zeros(
            [len(polarizations), n_samples, len(channels)], dtype=self.data_type
        )
            ...
            try:
                ...
                for polarization_idx in range(0, len(polarizations)):
                    ...
                    if polarization_name in file_groups_names:
                        polarization_grp = file_obj[
                            "polarization_" + str(current_polarization)
                        ]
                        if polarization_grp["data"].name == polarization_name + "/data":
                            ...
                            if (dset_columns == temp_dset.attrs["n_chans"]) and (
                                ...
                                for channel_idx in AAVSFileManager.range_array(
                                    0, len(channels)
                                ):
                                    current_channel = channels[channel_idx]
                                    if sample_offset + n_samples > nof_items:
                                        output_buffer[
                                            polarization_idx, 0:nof_items, channel_idx
                                        ] = dset[0:nof_items, current_channel]
                                    else:
                                        output_buffer[
                                            polarization_idx, :, channel_idx
                                        ] = dset[
    ...
    def _write_data(
        ...
        **kwargs
        ...
        file_obj = self.create_file(
            timestamp=timestamp, tile_id=station_id, partition_id=partition_id
        )
        file_obj.flush()
