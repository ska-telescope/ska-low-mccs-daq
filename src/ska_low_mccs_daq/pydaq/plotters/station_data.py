...
from ..persisters import FileDAQModes, StationBeamFormatFileManager
from .utils import *
...
def plot_station_beam_data(conf):
    ...
    if conf.plot_type in [PlotTypes.Phase, PlotTypes.ImagPart, PlotTypes.RealPart]:
        logging.error(
            "Selected plot type ({}) not valid for station data, exiting".format(
                conf.plot_type.name
            )
        )
        exit()
    ...
    station_plotter = StationBeamFormatFileManager(
    ...
    data, timestamps, _ = station_plotter.read_data(
    ...
    if len(params["channels"]) == 1:
        ...
        for pol in params["pols"]:
            ts = set_figure_for_timestamps(
                timestamps, conf.time, rollover=conf.rollover
            )
    ...
    # Generate spectrum plots if required
        ...
        if conf.separate and len(params["pols"]) > 1:
            fig, ax = plt.subplots(
                nrows=1, ncols=2, sharex="all", sharey="all", figsize=(12, 8)
            )
            fig.suptitle("Plotting {} ".format(conf.plot_type.name))
            for pol, col in enumerate(ax):
                col.plot(
                    frequencies, np.sum(data[pol, :, :], axis=0) // params["samples"]
                )
    ...
    # Generate waterfall plots
        ...
        # Otherwise plot each pol in a separate subplot
            ...
            # Add colorbar
            fig.subplots_adjust(
                bottom=0.1, top=0.9, left=0.1, right=0.88, wspace=0.02, hspace=0.02
            )
