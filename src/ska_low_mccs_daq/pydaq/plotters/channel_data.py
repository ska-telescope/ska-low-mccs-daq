...
def plot_channel_data(conf, integrated=False, continuous=False):
    ...
    if integrated:
        if conf.plot_type in [PlotTypes.Phase, PlotTypes.ImagPart, PlotTypes.RealPart]:
            logging.error(
                "Selected plot type ({}) not valid for integrated channel data, exiting".format(
                    conf.plot_type.name
                )
            )
            exit()
        ...
        if conf.log and conf.plot_type in [
            PlotTypes.Phase,
            PlotTypes.ImagPart,
            PlotTypes.RealPart,
        ]:
            logging.warning(
                "Log not supported for selected plot type ({}). Ignoring".format(
                    conf.plot_type.name
                )
            )
            conf.log = False
        ...
        if not conf.separate and conf.plot_type == PlotTypes.Waterfall:
            logging.info(
                "Selected plot type {} requires separate plots. Enabling separate plotting".format(
                    conf.plot_type.name
                )
            )
    ...
    if integrated:
        channel_plotter = ChannelFormatFileManager(
            root_path=conf.directory, daq_mode=FileDAQModes.Integrated
        )
    elif continuous:
        channel_plotter = ChannelFormatFileManager(
            root_path=conf.directory, daq_mode=FileDAQModes.Continuous
        )
    else:
        channel_plotter = ChannelFormatFileManager(
            root_path=conf.directory, daq_mode=FileDAQModes.Burst
        )
    ...
    if len(params["channels"]) == 1:
        if conf.plot_type in [PlotTypes.Spectrum, PlotTypes.Waterfall]:
            logging.warning(
                "Data files contains a single channel (or single channel requested), "
                "cannot generate waterfall or spectrum plot. Switching to magnitude"
            )
            conf.plot_type = PlotTypes.Magnitude
        elif conf.separate:
            logging.warning(
                "Separate plotting not supported for single channel plots. Ignoring"
            )
            conf.separate = False

    # Read data. Data is in channel/antenna/pol/sample order
    data, timestamps = channel_plotter.read_data(
    ...
    if len(params["channels"]) == 1:
        for p, pol in enumerate(params["pols"]):
            ...
            plt.title(
                "Tile {} - Polarisation {} - Channel {}. Plotting {}".format(
                    conf.tile_id,
                    pol_names[pol],
                    params["channels"][0],
                    conf.plot_type.name,
        ...
        if conf.separate:
            for p, pol in enumerate(params["pols"]):
                ...
                for a, antenna in enumerate(params["antennas"]):
                    ...
                    axes.set_title(
                        "Antenna: {}, RX: {}".format(antenna, get_rx(antenna))
                    )
        ...
        # Show spectra in same figure
            ...
            for p, pol in enumerate(params["pols"]):
                ...
                for a, antenna in enumerate(params["antennas"]):
                    ...
                    plt.title(
                        "Tile {} - Polarisation {}".format(conf.tile_id, pol_names[pol])
                    )
    ...
    # Generate waterfall plots
        ...
        for p, pol in enumerate(params["pols"]):
            fig, ax = plt.subplots(
                nrows=n_rows, ncols=n_cols, sharex="all", sharey="all", figsize=(12, 8)
            )
            fig.suptitle(
                "Polarisation {} (Plotting {}) ".format(
                    pol_names[pol], conf.plot_type.name
                )
            )
            ...
            for a, antenna in enumerate(params["antennas"]):
                ...
                im = axes.imshow(
                    to_plot,
                    aspect="auto",
                    extent=[frequencies[0], frequencies[-1], 0, to_plot.shape[0]],
                )
