...
from ..persisters import FileDAQModes, RawFormatFileManager
from .utils import *
...
def plot_raw_data(conf):
    ...
    if conf.plot_type != PlotTypes.Magnitude:
        logging.error(
            "Plot type {} not supported for raw data. Exiting".format(
                conf.plot_type.name
            )
        )
        exit(-1)
    ...
    raw_plotter = RawFormatFileManager(
    ...
    if conf.separate:
        for pol in params["pols"]:
            ...
            for antenna in range(len(params["antennas"])):
                ...
                axes.set_title(
                    "Antenna: {}, RX: {}".format(antenna_number, get_rx(antenna_number))
                )
                axes.grid(True)
