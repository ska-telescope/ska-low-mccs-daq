...
from ..persisters import BeamFormatFileManager, FileDAQModes
from .utils import *
...
def plot_beam_data(conf, integrated=False):
    ...
    if integrated:
        if conf.plot_type in [PlotTypes.Phase, PlotTypes.ImagPart, PlotTypes.RealPart]:
            logging.error(
                "Selected plot type ({}) not valid for integrated beam data, exiting".format(
                    conf.plot_type.name
                )
            )
            exit()
    ...
    if integrated:
        beam_plotter = BeamFormatFileManager(
    ...
    data, timestamps = beam_plotter.read_data(
