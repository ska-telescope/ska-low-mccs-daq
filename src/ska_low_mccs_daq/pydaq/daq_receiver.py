...
def get_ip_address(interface):
    ...
    return socket.inet_ntoa(
        fcntl.ioctl(
            s.fileno(), 0x8915, struct.pack("256s", interface[:15])  # SIOCGIFADDR
        )[20:24]
    ).encode()
...
def get_numpy_from_ctypes(pointer, dtype, nof_values):
    ...
    return np.frombuffer(
        value_buffer.from_address(ctypes.addressof(pointer.contents)), dtype
    )
...
def raw_data_callback(data, timestamp, tile, arg2):
    ...
    nof_values = (
        conf["nof_antennas"] * conf["nof_polarisations"] * conf["nof_raw_samples"]
    )
    ...
    if conf["raw_rms_threshold"] > -1:
        ...
        to_check = np.reshape(
            values.copy(),
            (conf["nof_antennas"], conf["nof_raw_samples"], conf["nof_polarisations"]),
        )
    ...
    filename = persisters[DaqModes.RAW_DATA].ingest_data(
...
def channel_data_callback(data, timestamp, tile, channel_id, mode="burst"):
    ...
    if mode == "continuous":
        ...
        if conf["continuous_period"] == 0:
            filename = persisters[DaqModes.CONTINUOUS_CHANNEL_DATA].ingest_data(
                append=True,
                data_ptr=values,
                timestamp=timestamps[DaqModes.CONTINUOUS_CHANNEL_DATA],
                sampling_time=sampling_time[DaqModes.CONTINUOUS_CHANNEL_DATA],
        ...
        if external_callbacks[DaqModes.CONTINUOUS_CHANNEL_DATA] is not None:
            external_callbacks[DaqModes.CONTINUOUS_CHANNEL_DATA](
        ...
        if conf["append_integrated"]:
            filename = persisters[DaqModes.INTEGRATED_CHANNEL_DATA].ingest_data(
                append=True,
                data_ptr=values,
                timestamp=timestamps[DaqModes.INTEGRATED_CHANNEL_DATA],
                sampling_time=sampling_time[DaqModes.INTEGRATED_CHANNEL_DATA],
                buffer_timestamp=timestamp,
                tile_id=tile,
            )
        else:
        ...
        if external_callbacks[DaqModes.INTEGRATED_CHANNEL_DATA] is not None:
            external_callbacks[DaqModes.INTEGRATED_CHANNEL_DATA](
        ...
        filename = persisters[DaqModes.CHANNEL_DATA].ingest_data(
            data_ptr=values,
            timestamp=timestamp,
            sampling_time=sampling_time[DaqModes.CHANNEL_DATA],
            tile_id=tile,
        )
        if conf["logging"]:
...
def beam_burst_data_callback(data, timestamp, tile, arg2):
    ...
    values = get_numpy_from_ctypes(
        ...
        conf["nof_beams"]
        * conf["nof_polarisations"]
        * conf["nof_beam_samples"]
        * conf["nof_beam_channels"],
    )

    filename = persisters[DaqModes.BEAM_DATA].ingest_data(
        data_ptr=values,
        timestamp=timestamp,
...
def beam_integrated_data_callback(data, timestamp, tile, arg2):
    ...
    # Extract data sent by DAQ
    values = get_numpy_from_ctypes(
        data, np.uint32, conf["nof_beams"] * conf["nof_polarisations"] * 384
    )

    # Re-arrange data
    values = np.reshape(values, (conf["nof_beams"], conf["nof_polarisations"], 384))
    ...
    filename = persisters[DaqModes.INTEGRATED_BEAM_DATA].ingest_data(
        append=conf["append_integrated"],
        data_ptr=values,
        timestamp=timestamps[DaqModes.INTEGRATED_BEAM_DATA],
        sampling_time=sampling_time[DaqModes.INTEGRATED_BEAM_DATA],
        buffer_timestamp=timestamp,
        tile_id=tile,
    )
...
def station_callback(data, timestamp, nof_packets, nof_saturations):
    ...
    # Extract data sent by DAQ
    values = get_numpy_from_ctypes(
        data, np.double, conf["nof_beam_channels"] * conf["nof_polarisations"]
    )
    ...
    filename = persisters[DaqModes.STATION_BEAM_DATA].ingest_data(
        append=True,
        data_ptr=values,
        timestamp=timestamps[DaqModes.STATION_BEAM_DATA],
        sampling_time=sampling_time[DaqModes.STATION_BEAM_DATA],
        buffer_timestamp=timestamp,
        station_id=0,
    ...
    if external_callbacks[DaqModes.STATION_BEAM_DATA] is not None:
        external_callbacks[DaqModes.STATION_BEAM_DATA](
...
def antenna_buffer_callback(data, timestamp, tile_id, _):
    ...
    filename = persisters[DaqModes.ANTENNA_BUFFER].ingest_data(
        append=True,
        data_ptr=values,
...
def start_raw_data_consumer(callback=None):
    ...
    # Start raw data consumer
    if (
        start_consumer("rawdata", params, callbacks[DaqModes.RAW_DATA])
        != Result.Success
    ):
        if conf["logging"]:
...
def start_channel_data_consumer(callback=None):
    ...
    # Start channel data consumer
    if (
        start_consumer("burstchannel", params, callbacks[DaqModes.CHANNEL_DATA])
        != Result.Success
    ):
...
def start_continuous_channel_data_consumer(callback=None):
    ...
    params = {
        ...
        // (
            sampling_time[DaqModes.CONTINUOUS_CHANNEL_DATA]
            * conf["nof_channel_samples"]
        ),
    ...
    if (
        start_consumer(
            "continuouschannel", params, callbacks[DaqModes.CONTINUOUS_CHANNEL_DATA]
        )
        != Result.Success
    ):
...
def start_integrated_channel_data_consumer(callback=None):
    ...
    if (
        start_consumer(
            "integratedchannel", params, callbacks[DaqModes.INTEGRATED_CHANNEL_DATA]
        )
        != Result.Success
    ):
        raise Exception("Failed to start continuous channel data consumer")
...
def start_beam_data_consumer(callback=None):
    ...
    if (
        start_consumer("burstbeam", params, callbacks[DaqModes.BEAM_DATA])
        != Result.Success
    ):
    ...
    beam_file.set_metadata(
        n_chans=conf["nof_beam_channels"],
        n_pols=conf["nof_polarisations"],
...
def start_integrated_beam_data_consumer(callback=None):
    ...
    if (
        start_consumer(
            "integratedbeam", params, callbacks[DaqModes.INTEGRATED_BEAM_DATA]
        )
        != Result.Success
    ):
        raise Exception("Failed to start beam data consumer")
...
def start_station_beam_data_consumer(callback=None):
    ...
    if (
        start_consumer("stationdata", params, callbacks[DaqModes.STATION_BEAM_DATA])
        != Result.Success
    ):
    ...
    # Set sampling time
    sampling_time[DaqModes.STATION_BEAM_DATA] = (1.0 / conf["sampling_rate"]) * conf[
        "nof_station_samples"
    ]
...
def start_antenna_buffer_data_consumer(callback=None):
    ...
    # Start raw data consumer
    if (
        start_consumer("antennabuffer", params, callbacks[DaqModes.ANTENNA_BUFFER])
        != Result.Success
    ):
        if conf["logging"]:
...
def populate_configuration(configuration):
    ...
    if not os.path.exists(configuration["directory"]):
        ...
        raise Exception(
            "Specified data directory [%s] does not exist" % configuration["directory"]
        )
    ...
    if conf["station_config"] is not None:
        if os.path.exists(conf["station_config"]) and os.path.isfile(
            conf["station_config"]
        ):
...
def initialise_daq(filepath=None):
    ...
    if (
        call_start_receiver(
            conf["receiver_interface"].encode(),
            conf["receiver_ip"],
            conf["receiver_frame_size"],
            conf["receiver_frames_per_block"],
            conf["receiver_nof_blocks"],
        )
...
def stop_daq():
    ...
    stop_functions = {
        DaqModes.RAW_DATA: stop_raw_data_consumer,
...
# Callbacks
callbacks = {
    DaqModes.RAW_DATA: DATA_CALLBACK(raw_data_callback),
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        type="int",
        default=32,
        help="Receiver frame size [default: 32]",
    )
    parser.add_option(
        "",
        "--receiver_nof_blocks",
        ...
        type="int",
        default=256,
        help="Receiver frame size [default: 256]",
    )
