...
class DaqReceiver:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None) -> None:
        ...
        self._callbacks = {
            ...
            DaqModes.BEAM_DATA: self.DYNAMIC_DATA_CALLBACK(
                self._beam_burst_data_callback
            ),
            ...
            DaqModes.STATION_BEAM_DATA: self.DYNAMIC_DATA_CALLBACK(
                self._station_callback
            ),
            DaqModes.ANTENNA_BUFFER: self.DYNAMIC_DATA_CALLBACK(
                self._antenna_buffer_callback
            ),
    ...
    def _raw_data_callback(
        ...
        if self._external_callbacks[DaqModes.RAW_DATA] is not None:
            self._external_callbacks[DaqModes.RAW_DATA](
                "burst_raw", filename, tile, spead_metadata=metadata
            )
    ...
    def _beam_burst_data_callback(
        ...
        if self._external_callbacks[DaqModes.BEAM_DATA] is not None:
            self._external_callbacks[DaqModes.BEAM_DATA](
                "burst_beam", filename, tile, spead_metadata=metadata
            )
    ...
    def _tc_correlator_callback(
        ...
        # Reshape data such that XY/YX stokes are inverted. This is to be consistent
        # with the stokes ordering the calibration architecture expects.
        data[:, [1, 2]] = data[:, [2, 1]]
    ...
    def _station_callback(
        ...
        nof_subarrays = len(self._config["subarray_ids"] or []) or 1
        values = self._get_numpy_from_ctypes(
            data,
            np.double,
    ...
    def _antenna_buffer_callback(
        ...
        metadata = ctypes.cast(
            metadata, ctypes.POINTER(self.AntennaBufferMetadata)
        ).contents
        tile = metadata.tile_id
        # Extract data sent by DAQ
        nof_values = (
    ...
    def _start_raw_data_consumer(self, callback: Optional[Callable] = None) -> None:
        ...
        if (
            self._start_consumer(
                "rawdata",
                params,
                self._callbacks[DaqModes.RAW_DATA],
    ...
    def _start_beam_data_consumer(self, callback: Optional[Callable] = None) -> None:
        ...
        if (
            self._start_consumer(
                "burstbeam",
                params,
                self._callbacks[DaqModes.BEAM_DATA],
                dynamic_callback=True,
            )
            != self.Result.Success
        ):
    ...
    def _start_integrated_beam_data_consumer(
        ...
        if (
            self._start_consumer(
                "integratedbeam",
                params,
                self._callbacks[DaqModes.INTEGRATED_BEAM_DATA],
                dynamic_callback=True,
            )
            != self.Result.Success
        ):
    ...
    def _start_station_beam_data_consumer(
        ...
        subarray_ids = self._config["subarray_ids"]
        if subarray_ids:
            params["subarray_ids"] = list(subarray_ids)

        if (
            self._start_consumer(
                "stationdata",
                params,
                self._callbacks[DaqModes.STATION_BEAM_DATA],
                dynamic_callback=True,
            )
            != self.Result.Success
        ):
        ...
        beam_file_mgr.set_metadata(
            n_chans=(len(subarray_ids or []) or 1) * self._config["nof_beam_channels"],
            n_pols=self._config["nof_polarisations"],
            n_samples=1,
            station_id=self._config["station_id"],
            station_name=self._config["station_name"],
    ...
    def _start_antenna_buffer_data_consumer(
        ...
        if (
            self._start_consumer(
                "antennabuffer",
                params,
                self._callbacks[DaqModes.ANTENNA_BUFFER],
                dynamic_callback=True,
            )
            != self.Result.Success
        ):
    ...
    def _start_station_beam_acquisition(self):
        ...
        params = {
            ...
            "station_beam_receiver_port": self._config["station_beam_receiver_port"],
