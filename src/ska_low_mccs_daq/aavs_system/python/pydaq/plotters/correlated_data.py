...
from ..persisters import CorrelationFormatFileManager, FileDAQModes
from .utils import *
...
def plot_correlated_data(conf):
    ...
    if conf.plot_type in [PlotTypes.Spectrum, PlotTypes.Waterfall]:
        logging.error(
            "Plot type {} not supported for correlated data. Exiting".format(
                conf.plot_type.name
            )
        )
        exit(-1)
    ...
    corr_plotter = CorrelationFormatFileManager(
    ...
    for pol in params["pols"]:
        ...
        grid = np.zeros(
            (len(params["antennas"]), len(params["antennas"])), dtype=np.complex64
        )
        ...
        plt.title(
            "Stokes {}. Plotting {}".format(
                "XX" if pol == 0 else "YY", conf.plot_type.name
            )
        )
        plt.xlabel("Antenna")
        plt.ylabel("Antenna")
