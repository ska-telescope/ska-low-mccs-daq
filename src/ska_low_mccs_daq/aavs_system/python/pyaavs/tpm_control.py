...
csp_ingest_network = {
    0: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    1: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    2: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    3: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    4: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    5: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    6: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
    7: {"mac": 0x0, "ip": "0.0.0.0", "port": 0xF0D1},
}
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="test",
        default=False,
        help="Load test firmware (-P still required) [default: False]",
    )
    parser.add_option(
        "-I",
        "--initialise",
    ...
    parser.add_option(
        ...
        dest="simulation",
        default=False,
        help="Connect to TPM in simulation mode [default: False]",
    )
    parser.add_option(
        "-A",
        "--enable-ada",
    ...
    parser.add_option(
        ...
        type="float",
        default=-1,
        help="Integrated channel integration time [default: -1 (disabled)]",
    )
    parser.add_option(
        "",
        "--beam-integration-time",
        action="store",
        dest="beam_integ",
        type="float",
        default=-1,
        help="Integrated beam integration time [default: -1 (disabled)]",
    )
    parser.add_option(
        "--ada-gain",
        action="store",
    ...
    parser.add_option(
        ...
        default=2,
        type="int",
        help="Channelsier truncation scale [range: 0-7, default: 2]",
    )
    parser.add_option(
        "",
        "--use_teng",
        ...
        dest="use_teng",
        default=False,
        help="Use 10G for LMC (default: False)",
    )
    ...
    if conf.program:
        ...
        if conf.bitfile is not None:
            ...
            if os.path.exists(conf.bitfile) and os.path.isfile(conf.bitfile):
                ...
                logging.error(
                    "Could not load bitfile %s, check filepath" % conf.bitfile
                )
                exit(-1)
        else:
