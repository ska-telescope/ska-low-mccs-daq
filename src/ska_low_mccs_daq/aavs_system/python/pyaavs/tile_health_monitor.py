...
from .tpm_monitoring_point_lookup import load_monitoring_point_lookup
...
class TileHealthMonitor:
    ...
    def enable_health_monitoring(self):
        ...
        if not all(communication_status.values()):
            raise BoardError(
                f"Board communication error, unable to enable health monitoring. Check communication status and try again."
            )
            ...
            if monitoring_point.startswith(path):
                ...
                for key, value in kwargs.items():
                    ...
                    if override or key not in lookup_entry:
                        self.logger.info(
                            f"Setting {key} for {monitoring_point} to {', '.join(value)}."
                        )
                        lookup_entry[key] = copy(value)
                    else:
                        self.logger.info(
                            f"Appending {key} for {monitoring_point} with {', '.join(value)}."
                        )
                        lookup_entry[key].extend(value)
                        lookup_entry[key] = copy(
                            list(set(lookup_entry[key]))
                        )  # remove duplicates from list by converting to set and back
                        self.logger.info(
                            f"{key} for {monitoring_point} now {lookup_entry[key]}."
                        )
        return
    ...
    def get_health_status(self, **kwargs):
        ...
        if not (communication_status["FPGA0"] and communication_status["FPGA1"]):
            ...
            self.logger.warning(
                f"Not able to communicate with one of more FPGAs. Reduced health status will be returned."
            )
        health_status = {}
        ...
        for clear_method in clear_method_list:
            try:
                ...
                self.logger.error(
                    f"Unable to clear monitoring_point {monitoring_point} "
                    "Exception : {e}"
                )
    ...
    def get_fpga_temperature(self, fpga_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            if self.is_programmed():
                temperature_dict[f"FPGA{fpga}"] = round(
                    self.tpm.tpm_sysmon[fpga].get_fpga_temperature(), 2
                )
            else:
    ...
    def get_available_voltages(self, fpga_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            available_voltages.extend(
                self.tpm.tpm_sysmon[fpga].get_available_voltages()
            )
    ...
    def get_tpm_temperature_thresholds(self):
        ...
        """
        return {
            "board_warning_threshold": self.tpm.tpm_monitor[
                0
            ].get_board_warn_temp_thresholds(),
            "board_alarm_threshold": self.tpm.tpm_monitor[
                0
            ].get_board_alm_temp_thresholds(),
            "fpga1_warning_threshold": self.tpm.tpm_monitor[
                0
            ].get_fpga_warn_temp_thresholds(fpga_id=0),
            "fpga1_alarm_threshold": self.tpm.tpm_monitor[
                0
            ].get_fpga_alm_temp_thresholds(fpga_id=0),
            "fpga2_warning_threshold": self.tpm.tpm_monitor[
                0
            ].get_fpga_warn_temp_thresholds(fpga_id=1),
            "fpga2_alarm_threshold": self.tpm.tpm_monitor[
                0
            ].get_fpga_alm_temp_thresholds(fpga_id=1),
        }
    ...
    def set_tpm_temperature_thresholds(
        ...
        if board_alarm_threshold is not None:
            if not (
                _is_in_range_20_50(board_alarm_threshold[0])
                and _is_in_range_20_50(board_alarm_threshold[1])
            ):
                raise ValueError(
        ...
        if fpga1_alarm_threshold is not None:
            if not (
                _is_in_range_20_50(fpga1_alarm_threshold[0])
                and _is_in_range_20_50(fpga1_alarm_threshold[1])
            ):
                raise ValueError(
        ...
        if fpga2_alarm_threshold is not None:
            if not (
                _is_in_range_20_50(fpga2_alarm_threshold[0])
                and _is_in_range_20_50(fpga2_alarm_threshold[1])
            ):
                raise ValueError(
        ...
        if board_alarm_threshold is not None:
            self.tpm.tpm_monitor[0].set_board_alm_temp_thresholds(
                board_alarm_threshold[0], board_alarm_threshold[1]
            )
        if fpga1_alarm_threshold is not None:
            self.tpm.tpm_monitor[0].set_fpgas_alm_temp_thresholds(
                fpga1_alarm_threshold[0], fpga1_alarm_threshold[1], fpga_id=0
            )
        if fpga2_alarm_threshold is not None:
            self.tpm.tpm_monitor[0].set_fpgas_alm_temp_thresholds(
                fpga2_alarm_threshold[0], fpga2_alarm_threshold[1], fpga_id=1
            )
    ...
    def get_available_currents(self, fpga_id=None):
        ...
        # System Monitor Plugin
        for fpga in self.fpga_gen(fpga_id):
            available_currents.extend(
                self.tpm.tpm_sysmon[fpga].get_available_currents()
            )
    ...
    def check_adc_sysref_setup_and_hold(self, adc_id=None, show_info=True):
        ...
        case_dict = {
            "case1": {
                "hold": [0x0],
                "setup": [0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7],
                "status": False,
                "msg": "Possible setup error.The smaller this number, the smaller the setup margin.",
            },
            "case2": {
                "hold": [0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8],
                "setup": [0x8],
                "status": True,
                "msg": "No setup or hold error (best hold margin).",
            },
            "case3": {
                "hold": [0x8],
                "setup": [0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF],
                "status": True,
                "msg": "No setup or hold error (best setup and hold margin).",
            },
            "case4": {
                "hold": [0x8],
                "setup": [0x0],
                "status": True,
                "msg": "No setup or hold error (best setup margin).",
            },
            "case5": {
                "hold": [0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF],
                "setup": [0x0],
                "status": False,
                "msg": "Possible hold error. The larger this number the smaller the hold margin.",
            },
            "case6": {
                "hold": [0x0],
                "setup": [0x0],
                "status": False,
                "msg": "Possible setup or hold error.",
            },
        }
        adcs = range(16) if adc_id is None else [adc_id]
        status_dict = {}
        for adc in adcs:
            ...
            for case in case_dict.values():
                if hold in case["hold"] and setup in case["setup"]:
                    if show_info:
                        self.logger.info(
                            f"ADC{adc} {case['msg']} Setup: {hex(setup)}, Hold {hex(hold)}."
                        )
                ...
                if show_info:
                    self.logger.error(
                        f"ADC{adc} Invalid Setup and Hold values. Setup: {hex(setup)}, Hold {hex(hold)}."
                    )
        ...
        for adc in adcs:
            ...
            while True:
                ...
                if time.time() > timeout:
                    raise BoardError(
                        f"Timed out trying to read ADC{adc} SYSREF counter - 0x012A twice in under 3 ms."
                    )
    ...
    def check_clock_status(self, fpga_id=None, clock_name=None):
        ...
        if self.is_programmed():
            result = {}
            for fpga in self.fpga_gen(fpga_id):
                result[f"FPGA{fpga}"] = self.tpm.tpm_clock_monitor[
                    fpga
                ].check_clock_status(clock_name)
            return result
        return
    ...
    def check_clock_manager_status(self, fpga_id=None, name=None):
        ...
        status = {}
        for fpga in self.fpga_gen(fpga_id):
            status[f"FPGA{fpga}"] = self.tpm.tpm_clock_monitor[
                fpga
            ].check_clock_manager_status(name)
        return status
    ...
    def check_jesd_lane_error_counter(self, fpga_id=None, core_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            ...
            for core in cores:
                idx = fpga * jesd_cores_per_fpga + core
                counter_dict[f"FPGA{fpga}"][f"Core{core}"] = self.tpm.tpm_jesd[
                    idx
                ].check_link_error_counter()
        return counter_dict
    ...
    def check_jesd_resync_counter(self, fpga_id=None, show_result=True):
        ...
        for fpga in self.fpga_gen(fpga_id):
            ...
            counts[f"FPGA{fpga}"] = self.tpm.tpm_jesd[idx].check_resync_counter(
                show_result
            )
    ...
    def check_jesd_qpll_status(self, fpga_id=None, show_result=True):
        ...
        for fpga in self.fpga_gen(fpga_id):
            ...
            lock_loss_cnt = self.tpm.tpm_jesd[idx].check_qpll_lock_loss_counter(
                show_result
            )
    ...
    def check_ddr_reset_counter(self, fpga_id=None, show_result=True):
        ...
        counts = {}
        for fpga in self.fpga_gen(fpga_id):
            counts[f"FPGA{fpga}"] = self.tpm.tpm_test_firmware[
                fpga
            ].check_ddr_user_reset_counter(show_result)
        return counts  # Return dict of counter values
    ...
    def check_ddr_parity_error_counter(self, fpga_id=None):
        ...
        counts = {}
        for fpga in self.fpga_gen(fpga_id):
            counts[f"FPGA{fpga}"] = self.tpm.station_beamf[
                fpga
            ].check_ddr_parity_error_counter()
        return counts
    ...
    def check_f2f_pll_status(self, core_id=None, show_result=True):
        ...
        counts = {}
        for core in cores:
            counts[f"Core{core}"] = self.tpm.tpm_f2f[core].check_pll_lock_status(
                show_result
            )
        return counts  # Return dict of counter values
    ...
    def check_data_router_status(self, fpga_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            output_dict["status"][f"FPGA{fpga}"] = self.tpm.tpm_test_firmware[
                fpga
            ].check_data_router_status()
            output_dict["discarded_packets"][f"FPGA{fpga}"] = (
                self.tpm.tpm_test_firmware[fpga].check_data_router_discarded_packets()
            )
    ...
    def check_udp_arp_table_status(self, fpga_id=None, show_result=True):
        ...
        arp_table_ids = range(
            self.tpm.tpm_10g_core[0].get_number_of_arp_table_entries()
        )
        ...
        for fpga in self.fpga_gen(fpga_id):
            if self.active_40g_port[fpga]:  # Ignore ARP table if 40G QSFP not in use
                ...
                for arp_table in arp_table_ids:
                    arp_status, mac = self.tpm.tpm_10g_core[fpga].get_arp_table_status(
                        arp_table, silent_mode
                    )
                    if arp_status & 0x1:
                        if arp_status & 0x4:
                            resolved_cnt += 1
        ...
        return (
            True
            if all(fpga_resolved_entries) and not any(fpga_unresolved_entries)
            else False
        )
    ...
    def check_udp_linkup_loss_counter(self, fpga_id=None, show_result=True):
        ...
        counts = {}
        for fpga in self.fpga_gen(fpga_id):
            counts[f"FPGA{fpga}"] = self.tpm.tpm_10g_core[fpga].check_linkup_loss_cnt(
                show_result
            )
        return counts  # Return dict of counter values
    ...
    def check_udp_bip_error_counter(self, fpga_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            if self.active_40g_port[fpga]:  # Ignore BIP errors if 40G QSFP not in use
                counts[f"FPGA{fpga}"] = self.tpm.tpm_10g_core[
                    fpga
                ].get_bip_error_count()
            else:
    ...
    def check_station_beamformer_status(self, fpga_id=None):
        ...
        if self.is_programmed():
            ...
            return not any(
                errors
            )  # Return True if all flags and counters are 0, else False
        return
    ...
    def check_station_beamformer_discarded_or_flagged_packet_count(self, fpga_id=None):
        ...
        for fpga in self.fpga_gen(fpga_id):
            counts[f"FPGA{fpga}"] = self.tpm.station_beamf[
                fpga
            ].get_discarded_or_flagged_packet_count()
        return counts  # Return dict of counter values
    ...
    def inject_ddr_parity_error(self, fpga_id=None):
        for fpga in self.fpga_gen(fpga_id):
            ...
            while True:
                ...
                if count > timeout:
                    self.logger.error(
                        "Timed out waiting for DDR parity error injection acknowledge. No valid DDR read transaction"
                    )
                    break
                time.sleep(0.5)
                count += 1
