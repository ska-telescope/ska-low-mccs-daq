...
from pyfabil.base.utils import format_data, ip2long
...
from .tile_health_monitor import TileHealthMonitor
...
class Tile(TileHealthMonitor):
    ...
    def info(self):
        ...
        info["fpga_firmware"] = {}
        info["fpga_firmware"]["design"] = self.tpm.tpm_firmware_information[
            0
        ].get_design()
        info["fpga_firmware"]["build"] = self.tpm.tpm_firmware_information[
            0
        ].get_build()
        info["fpga_firmware"]["compile_time"] = self.tpm.tpm_firmware_information[
            0
        ].get_time()
        info["fpga_firmware"]["compile_user"] = self.tpm.tpm_firmware_information[
            0
        ].get_user()
        info["fpga_firmware"]["compile_host"] = self.tpm.tpm_firmware_information[
            0
        ].get_host()
        info["fpga_firmware"]["git_branch"] = self.tpm.tpm_firmware_information[
            0
        ].get_git_branch()
        info["fpga_firmware"]["git_commit"] = self.tpm.tpm_firmware_information[
            0
        ].get_git_commit()
        info["fpga_firmware"]["version"] = self.tpm.tpm_firmware_information[
            0
    ...
    def initialise(
        ...
        src_port=4661,
        dst_port=4660,
        dst_port_single_port_mode=4662,
        rx_port_single_port_mode=4662,
        netmask_40g=None,
        gateway_ip_40g=None,
    ...
    def configure_active_40g_ports(self, configuration):
        ...
        if self.has_register("fpga1.dsp_regfile.config_id.is_master"):
            ...
            # If single 40G not supported by firmware both ports must be used
            self.logger.warning(
                "TPM firmware does not support different active 40G port configurations. Both 40G ports will be used."
            )
            return
        if configuration == "port1-only":
            ...
            self.logger.error(
                f"Invalid configuration {configuration} specifie. Options are: port1-only, port2-only, both-ports"
            )
        return
        ...
        if self["fpga1.regfile.feature.xg_eth_implemented"] == 1:
            ...
            for n in range(len(self.tpm.tpm_10g_core)):
                ...
                if cable_detected:
                    ...
                    self.configure_40g_core(
                        core_id=n, arp_table_entry=1, rx_port_filter=channel2_rx_port
                    )
                else:
        ...
        for c in core_id:
            if self.tpm.tpm_10g_core[c].is_tx_disabled():
                ...
                self.logger.warning(
                    "Skipping ARP table check on FPGA"
                    + str(c + 1)
                    + ". Port disabled for single active port mode!"
                )
        ...
        if self["fpga1.regfile.feature.xg_eth_implemented"] == 1:
            ...
            for core_id in core_ids:
                for arp_table in arp_table_ids:
                    core_status, core_mac = self.tpm.tpm_10g_core[
                        core_id
                    ].get_arp_table_status(arp_table, silent_mode=True)
    ...
    def set_beamformer_epoch(self, epoch):
        ...
        if self.spead_ska_format_supported:
            ...
            for fpga in ["fpga1", "fpga2"]:
                self.tpm[f"{fpga}.beamf_ring.ref_epoch_frame_hi"] = int(
                    csp_reference_frame >> 32
                )
                self.tpm[f"{fpga}.beamf_ring.ref_epoch_frame_lo"] = (
                    csp_reference_frame & 0xFFFFFFFF
                )
    ...
    def set_spead_format(self, ska_spead_header_format: bool):
        ...
        if self.spead_ska_format_supported:
            ...
            raise LibraryError(
                f"New spead header is not supported with this version of the firmware"
            )
        return
    ...
    def start_beamformer(self, start_time=0, duration=-1, scan_id=0, mask=0xFFFFFFFFFF):
        ...
        ret1 = self.tpm.station_beamf[0].start(
            start_time, duration, scan_id=scan_id, mask=mask
        )
        ...
        if time_now >= start_time:
            logging.error(
                "Tile start_beamformer error. Synchronised operation failed! Time difference: "
    ...
    def sync_fpga_time(self, use_internal_pps=False):
        ...
        for f in devices:
            self.tpm[f + ".pps_manager.pps_gen_tc"] = (
                int(100e6) - 1
            ...
            if self.tpm.has_register("fpga1.pps_manager.pps_exp_tc"):
                self.tpm[f + ".pps_manager.pps_exp_tc"] = (
                    int(200e6) - 1
    ...
    def start_acquisition(self, start_time=None, delay=2, global_start_time=None):
        ...
        if self.tpm.has_register(f"{fpga}.pps_manager.sync_time_actual_val"):
            global_sync_time = int(
                global_sync_time - (global_sync_time - tai_2000_epoch) % 864
            )
        else:
            global_sync_time = sync_time
