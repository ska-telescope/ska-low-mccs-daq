...
struct SplitSlot
{
    ...
    uint32_t               split_idx = 0;  // written before FILLING published; read after READY
...
public:
    ...
    // Returns false and drops the write if the slot is not yet free.
    bool write_data(uint32_t split_idx, uint16_t start_antenna, uint16_t nof_included,
